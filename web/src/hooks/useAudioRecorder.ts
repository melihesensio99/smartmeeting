...
export function useAudioRecorder() {
  ...
  const start = useCallback(async () => { const microphoneStream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true, channelCount: 1, sampleRate: 48000 } }); chunksRef.current = []; const recorder = new MediaRecorder(microphoneStream); recorder.ondataavailable = (event) => { if (event.data.size > 0) chunksRef.current.push(event.data) }; recorder.onstop = () => microphoneStream.getTracks().forEach((track) => track.stop()); recorder.start(1000); recorderRef.current = recorder; setStream(microphoneStream); setElapsedSeconds(0); setState('recording') }, [])
  const pause = useCallback(() => { const recorder = recorderRef.current; if (!recorder || recorder.state !== 'recording') return; recorder.pause(); setState('paused') }, [])
  const resume = useCallback(() => { const recorder = recorderRef.current; if (!recorder || recorder.state !== 'paused') return; recorder.resume(); setState('recording') }, [])
