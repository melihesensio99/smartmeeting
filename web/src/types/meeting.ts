import { z } from 'zod'
export const actionItemSchema = z.object({ description: z.string(), assignee: z.string().nullable(), dueAt: z.string().nullable(), completed: z.boolean() })
export const meetingSchema = z.object({ id: z.string().uuid(), title: z.string(), organizerId: z.string(), startsAt: z.string(), endsAt: z.string().nullable(), status: z.union([z.number(), z.string()]), transcript: z.string().nullable(), summary: z.object({ overview: z.string(), decisions: z.array(z.string()), actionItems: z.array(actionItemSchema) }).nullable() })
