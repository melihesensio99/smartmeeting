...
export function MeetingRoomMediaPanel({ localStream, remoteStreams, participants, isMuted, isCameraOff, onToggleMute, onToggleCamera }: Props) {
  ...
  return <Card><CardContent><Stack spacing={2}><Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { sm: 'center' }, gap: 1 }}><Box><Typography variant="h5">Sesli ve görüntülü görüşme</Typography><Typography variant="body2" color="text.secondary">WebRTC bağlantısı doğrudan katılımcılar arasında kurulur. Kamera başlangıçta kapalıdır.</Typography></Box><Stack direction="row" spacing={1}><Button size="small" variant="outlined" onClick={onToggleMute}>{isMuted ? 'Mikrofonu aç' : 'Mikrofonu kapat'}</Button><Button size="small" variant="outlined" onClick={() => void onToggleCamera()}>{isCameraOff ? 'Kamerayı aç' : 'Kamerayı kapat'}</Button></Stack></Stack><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 1.5 }}><>{localStream && <VideoTile stream={localStream} label={isCameraOff ? 'Siz · Kamera kapalı' : 'Siz'} muted />}{remoteEntries.map(([userId, stream]) => <VideoTile key={userId} stream={stream} label={participants.find((participant) => participant.userId === userId)?.displayName ?? 'Katılımcı'} />)}</></Box>{remoteEntries.length === 0 && <Typography variant="body2" color="text.secondary">Diğer katılımcılar kameralarını açtığında görüntüleri burada görünecek.</Typography>}</Stack></CardContent></Card>
}
