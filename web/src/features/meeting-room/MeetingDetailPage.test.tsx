...
describe("MeetingDetailPage işlem durumu", () => {
  it("processing durumunu ilerleme mesajıyla gösterir", () => {
    ...
    expect(
      screen.getByText("Ses dosyası, transkript ve AI özeti hazırlanıyor."),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Yeniden işle" }),
  ...
  it("failed durumunda yeniden işleme aksiyonunu tetikler", () => {
    ...
    expect(onRetryProcessing).toHaveBeenCalledWith(
      "11111111-1111-1111-1111-111111111111",
    );
  });

  it("toplantı sahibi katılımcı yönetimi ve konuşmacı onay kontrollerini görür", () => {
    const meeting = createMeeting(3, [
      {
        id: "44444444-4444-4444-4444-444444444444",
        userId: "participant-1",
        displayName: "Ayşe Katılımcı",
        email: "ayse@example.com",
        canManageMeeting: true,
        speakerLabel: "Speaker 1",
        speakerMappingStatus: "PendingConfirmation",
        speakerConfidence: 0.8,
      },
    ]);
    renderPage(meeting);
    fireEvent.click(
      screen.getByRole("button", { name: /Katılımcı ekle ve yetkiyi düzenle/ }),
    );

    expect(
      screen.getByRole("button", { name: "Katılımcı ekle" }),
    ).toBeDisabled();
  ...
  it("normal katılımcı toplantıdan ayrılabilir ancak katılımcı yönetemez", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Odaya gir" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Katılımcı ekle" }),
    ).not.toBeInTheDocument();
  });
});
