import { Alert, Box, Button, Card, CardContent, Chip, Divider, LinearProgress, MenuItem, Select, Stack, TextField, Typography } from '@mui/material'
...
export function MeetingDetailPage({ meeting, onComplete, onUpdateAction, onSaveNotes, onMapSpeaker, onAddParticipant, onSendEmail, savingNotes, addingParticipant, sendingEmail, updatingAction }: Props) {
  ...
  return <Stack spacing={3}>
    ...
    <Card><CardContent><Typography variant="h5">Katılımcılar ve konuşmacı eşleştirme</Typography><Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} sx={{ mt: 2, mb: 1 }}><TextField fullWidth size="small" label="Kayıtlı kullanıcı ara" placeholder="Ad soyad veya e-posta" value={participantSearch} onChange={(event) => { setParticipantSearch(event.target.value); setSelectedUser(null) }} /><Button variant="contained" disabled={!selectedUser || addingParticipant} onClick={() => { if (!selectedUser) return; const input: AddParticipantInput = selectedUser; onAddParticipant(meeting.id, input); setParticipantSearch(''); setSelectedUser(null) }}>{addingParticipant ? 'Ekleniyor…' : 'Katılımcı ekle'}</Button></Stack>{users.isFetching && <Typography color="text.secondary">Kullanıcılar aranıyor…</Typography>}{users.isError && <Alert severity="error">{getApiErrorMessage(users.error, 'Kullanıcılar getirilemedi.')}</Alert>}{users.data && users.data.length > 0 && <Stack spacing={1} sx={{ mb: 2 }}>{users.data.map((user) => <Button key={user.userId} variant={selectedUser?.userId === user.userId ? 'contained' : 'outlined'} onClick={() => setSelectedUser(user)} sx={{ justifyContent: 'flex-start', textAlign: 'left' }}>{user.displayName} · {user.email}</Button>)}</Stack>}<Typography color="text.secondary" sx={{ mt: 1, mb: 2 }}>Yalnızca sistemde kayıtlı kullanıcılar toplantıya eklenebilir. AI diarization etiketlerini kayıtlı katılımcılara bağlayın.</Typography><Stack spacing={2}>{meeting.participants.length ? meeting.participants.map((participant) => <Stack key={participant.id} direction={{ xs: 'column', md: 'row' }} sx={{ gap: 2, alignItems: { md: 'center' } }}><Typography sx={{ minWidth: 180, fontWeight: 700 }}>{participant.displayName}</Typography><Select size="small" displayEmpty value={speakerLabels[participant.id] ?? participant.speakerLabel ?? ''} onChange={(event) => setSpeakerLabels((current) => ({ ...current, [participant.id]: event.target.value }))} sx={{ minWidth: 180 }}><MenuItem value=""><em>Speaker etiketi seçin</em></MenuItem>{Array.from({ length: 8 }, (_, index) => <MenuItem key={index} value={`Speaker ${index + 1}`}>Speaker {index + 1}</MenuItem>)}</Select><Button variant="outlined" disabled={!speakerLabels[participant.id]?.trim()} onClick={() => onMapSpeaker(meeting.id, participant.id, speakerLabels[participant.id])}>Eşleştir</Button></Stack>) : <Alert severity="info">Bu toplantıya henüz katılımcı eklenmemiş.</Alert>}</Stack></CardContent></Card>
