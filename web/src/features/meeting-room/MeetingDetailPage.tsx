...
export function MeetingDetailPage({ meeting, currentUserId, onComplete, onUpdateAction, onCreateAction, onSaveNotes, onRecordingStart, onAudioReady, recordingUploading, recordingStarting, onMapSpeaker, onConfirmSpeaker, onRejectSpeaker, onAddParticipant, onUpdateParticipantPermission, onRemoveParticipant, onLeaveMeeting, onSendEmail, onRetryProcessing, savingNotes, addingParticipant, updatingParticipantPermission, removingParticipant, leavingMeeting, confirmingSpeaker, rejectingSpeaker, sendingEmail, updatingAction, creatingAction, retryingProcessing, emailSent, emailError }: Props) {
  ...
  const statusKey = ({ Scheduled: '0', Recording: '1', Processing: '2', Ready: '3', Failed: '4', Cancelled: '5' } as Record<string, string>)[status] ?? status
  ...
  return <Stack spacing={3}>
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} sx={{ justifyContent: 'space-between', gap: 2 }}><Box><Typography variant="h3">{meeting.title}</Typography><Typography color="text.secondary">{new Date(meeting.startsAt).toLocaleString('tr-TR')} · Düzenleyen: {meeting.organizerId}</Typography></Box><Button variant="contained" onClick={() => document.getElementById('meeting-recording')?.scrollIntoView({ behavior: 'smooth' })}>Kayıt paneline git</Button></Stack>
    <Card sx={{ border: '1px solid', borderColor: roomPresence.isInRoom ? 'success.main' : 'divider' }}><CardContent><Stack spacing={2}>{roomPresence.isInRoom && <Alert severity="success">● Şu an bu toplantının canlı odasındasınız.</Alert>}<Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { sm: 'center' }, gap: 2 }}><Box><Typography variant="h5">Canlı toplantı odası</Typography><Typography color="text.secondary">Odaya girerek toplantıya katılan kişileri gerçek zamanlı görebilirsiniz.</Typography></Box><Button variant={roomPresence.isInRoom ? 'outlined' : 'contained'} color={roomPresence.isInRoom ? 'error' : 'primary'} disabled={roomPresence.isConnecting} onClick={() => void (roomPresence.isInRoom ? roomPresence.leaveRoom() : roomPresence.enterRoom())}>{roomPresence.isConnecting ? 'Bağlanıyor…' : roomPresence.isInRoom ? 'Odadan çık' : 'Odaya gir'}</Button></Stack>{roomPresence.error && <Alert severity="error">{roomPresence.error}</Alert>}{roomPresence.isInRoom ? <><Stack direction="row" spacing={1} sx={{ alignItems: 'center', flexWrap: 'wrap' }}><Chip color="success" label={`${roomPresence.participants.length} kişi şu an odada`} />{roomPresence.participants.map((participant) => <Chip key={participant.userId} variant="outlined" label={`${participant.displayName}${participant.isOrganizer ? ' · Yönetici' : ''}`} />)}</Stack><Typography variant="caption" color="text.secondary">Katılımcıların giriş ve çıkışları bu alanda anlık güncellenir.</Typography></> : <Typography variant="body2" color="text.secondary">Henüz odaya girmediniz. Odaya girdiğinizde diğer aktif katılımcılar görünür.</Typography>}</Stack></CardContent></Card>
