...
export function useMeetingRoomPresence(meetingId: string | undefined) {
  ...
  const sendSignal = useCallback(
    async (
      targetUserId: string,
      signalType: RoomSignal["signalType"],
  ...
  const createPeer = useCallback(
    (userId: string) => {
      ...
      peer.onicecandidate = (event) => {
        if (event.candidate)
          void sendSignal(userId, "ice", event.candidate.toJSON());
      };
      peer.ontrack = (event) => {
  ...
  const enterRoom = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Mikrofon ve kamera erişimi verilemedi. Tarayıcı izinlerini kontrol edin.",
      );
    ...
    connection.on("roomPresenceChanged", (snapshot: RoomPresenceEvent) =>
      setParticipants(snapshot),
    );
    connection.on("roomParticipantLeft", ({ userId }: { userId: string }) =>
      closePeer(userId),
    );
    connection.on(
    ...
    connection.on(
      "webrtcSignal",
      async ({ fromUserId, signalType, payload }: RoomSignal) => {
        const peer = createPeer(fromUserId);
        const signal = JSON.parse(payload) as RTCSessionDescriptionInit &
          RTCIceCandidateInit;
        if (signalType === "offer") {
    ...
    try {
      ...
      const snapshot = await connection.invoke<RoomPresenceEvent>(
        "JoinRoom",
        meetingId,
      );
      setParticipants(snapshot);
      ...
      await leaveRoom();
      setError(
        "Toplantı odasına bağlanılamadı. Yetkinizi ve API bağlantısını kontrol edin.",
      );
    } finally {
      setIsConnecting(false);
    }
    ...
    if (!localStreamRef.current?.getVideoTracks().length) {
      try {
        ...
        Object.values(peersRef.current).forEach((peer) =>
          peer.addTrack(cameraTrack, localStreamRef.current as MediaStream),
        );
        setLocalStream(new MediaStream(localStreamRef.current.getTracks()));
        setIsCameraOff(false);
      } catch {
        setError(
          "Kamera erişimi verilemedi. Kamera kapalı şekilde devam edebilirsiniz.",
        );
      }
      return;
    }
