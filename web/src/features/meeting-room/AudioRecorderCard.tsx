...
export function AudioRecorderCard({ meetings, selectedMeetingId, onMeetingChange, onRecordingStart, onAudioReady, onSaveNotes, uploading, starting, savingNotes, fixedMeeting, canRecord = true, hideActionButton = false, compact = false, onRecordingStateChange, onRecordingDurationChange }: Props) {
  ...
  return <Card sx={{ height: '100%' }}><CardContent><Stack spacing={2}>
    {!isCompact && <><Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center', gap: 1 }}><Typography variant="h6">Sesli toplantı kaydı</Typography>{selectedMeeting && !fixedMeeting && <Button size="small" variant="outlined" onClick={() => navigate(`/meetings/${selectedMeeting.id}`)}>Canlı odayı aç</Button>}</Stack>{fixedMeeting ? <Chip label={`Toplantı: ${selectedMeeting?.title ?? 'Seçilmedi'}`} variant="outlined" sx={{ alignSelf: 'flex-start' }} /> : <FormControl fullWidth disabled={recorder.state === 'recording' || uploading}><InputLabel id="meeting-select-label">Toplantı</InputLabel><Select labelId="meeting-select-label" label="Toplantı" value={selectedMeetingId} onChange={(event) => handleMeetingChange(event.target.value)}><MenuItem value=""><em>Toplantı seçin</em></MenuItem>{meetings.filter((meeting) => ['0', 'Scheduled'].includes(String(meeting.status)) || meeting.id === selectedMeetingId).map((meeting) => <MenuItem key={meeting.id} value={meeting.id}>{meeting.title}</MenuItem>)}</Select></FormControl>}</>}
    {!isCompact && <Typography color="text.secondary">Kayıt, toplantı odasına katıldıktan sonra kullanılabilir. Katılımcılar ve kamera/mikrofon bağlantısı canlı oda bölümünden yönetilir.</Typography>}
    {selectedMeeting && <Stack spacing={1.5} sx={isCompact ? { p: 2, borderRadius: 2, border: '2px solid', borderColor: 'secondary.main', bgcolor: 'background.paper' } : undefined}>{isCompact && <Typography variant="h6" sx={{ fontWeight: 800 }}>Canlı toplantı notları</Typography>}<TextField fullWidth multiline minRows={isCompact ? 5 : 3} label={isCompact ? 'Not ekle' : 'Canlı toplantı notları'} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Toplantı sırasında önemli kararları veya bağlamı yazın..." disabled={uploading} /><Button variant="outlined" onClick={() => onSaveNotes(selectedMeeting.id, notes)} disabled={!notes.trim() || savingNotes}>{savingNotes ? 'Not kaydediliyor…' : 'Notları kaydet'}</Button><Typography variant="caption" color="text.secondary">Bu notlar STT tamamlandıktan sonra AI özetine bağlam olarak dahil edilir.</Typography></Stack>}
    {!isCompact && !selectedMeeting && <Alert severity="info">Kayıt başlatmak için planlanmış bir toplantı seçin.</Alert>}
    {!isCompact && selectedMeeting && !canRecord && <Alert severity="info">Kayıt başlatmak için önce canlı toplantı odasına girin.</Alert>}
    {!isCompact && recorder.state === 'recording' && <Alert severity="warning">Kayıt devam ediyor</Alert>}
    {!isCompact && <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between', gap: 2, p: 2, borderRadius: 2, bgcolor: 'grey.100' }}><Typography variant="h4" sx={{ fontVariantNumeric: 'tabular-nums', letterSpacing: 1 }}>{duration}</Typography><Typography color="text.secondary">Kayıt süresi</Typography></Stack>}
    {!isCompact && <Stack direction="row" sx={{ height: 72, alignItems: 'center', justifyContent: 'center', gap: 0.5, px: 1, borderRadius: 2, bgcolor: 'primary.dark' }} aria-label="Ses frekans görselleştirmesi">{levels.map((level, index) => <Box key={index} component="span" sx={{ width: 4, height: `${Math.max(8, level * 56)}px`, borderRadius: 1, bgcolor: 'secondary.main', transition: 'height 80ms linear' }} />)}</Stack>}
