...
type Props = {
  ...
  onRecordingDurationChange?: (duration: string) => void;
  onControllerReady?: (controller: AudioRecorderController | null) => void;
};
...
export function AudioRecorderCard({
  ...
  useEffect(() => {
    onControllerReady?.({ toggle, togglePause, stop: stopRecording });
    return () => onControllerReady?.(null);
  }, [onControllerReady, stopRecording, toggle, togglePause]);
  const duration = [
    Math.floor(recorder.elapsedSeconds / 3600),
    Math.floor((recorder.elapsedSeconds % 3600) / 60),
    recorder.elapsedSeconds % 60,
  ]
        ...
        <Stack spacing={3}>
          {!isCompact && (
            <>
              ...
              {fixedMeeting ? (
                <Chip
                  label={`Toplantı: ${selectedMeeting?.title ?? "Seçilmedi"}`}
                  variant="outlined"
                  sx={{
                ...
                >
                  <InputLabel id="meeting-select-label">Toplantı</InputLabel>
                  <Select
                    ...
                    sx={{ borderRadius: 3 }}
                    onChange={(event) =>
                      handleMeetingChange(event.target.value)
                    }
                  >
                    <MenuItem value="">
                      <em>Toplantı seçin</em>
                    </MenuItem>
                    {meetings
                      .filter(
                        (meeting) =>
                          ["0", "Scheduled"].includes(String(meeting.status)) ||
          ...
          {!isCompact && (
            <Typography color="text.secondary" sx={{ lineHeight: 1.6 }}>
              Kayıt, toplantı odasına katıldıktan sonra kullanılabilir.
              Katılımcılar ve kamera/mikrofon bağlantısı canlı oda bölümünden
              yönetilir.
            </Typography>
          )}
          {selectedMeeting && (
              ...
              sx={
                isCompact
                  ? {
                      ...
                      borderRadius: 4,
                      background: (theme) =>
                        `linear-gradient(${theme.palette.background.paper}, ${theme.palette.background.paper}) padding-box, linear-gradient(135deg, ${alpha(theme.palette.primary.main, 0.5)}, ${alpha(theme.palette.secondary.main, 0.5)}) border-box`,
            ...
            >
              ...
              <TextField
                fullWidth
                multiline
                minRows={isCompact ? 5 : 3}
                label={isCompact ? "Not ekle" : "Canlı toplantı notları"}
                value={notes}
              ...
              <Button
                ...
                onClick={() => onSaveNotes(selectedMeeting.id, notes)}
          ...
          {!isCompact && !selectedMeeting && (
            <Alert severity="info" sx={{ borderRadius: 3 }}>
              Kayıt başlatmak için planlanmış bir toplantı seçin.
            </Alert>
          )}
          {!isCompact && selectedMeeting && !canRecord && (
            <Alert severity="info" sx={{ borderRadius: 3 }}>
              Kayıt başlatmak için önce canlı toplantı odasına girin.
            </Alert>
          )}
          {!isCompact && recorder.state === "recording" && (
            ...
            >
              <Typography
                ...
                sx={{
                  ...
                  letterSpacing: 2,
                  background: (theme) =>
                    `linear-gradient(to right, ${theme.palette.secondary.main}, #fff)`,
          ...
          {!isCompact && (
            <Stack
              ...
              aria-label="Ses frekans görselleştirmesi"
            >
              {levels.map((level, index) => (
                <Box
                  ...
                  sx={{
                    ...
                    boxShadow: (theme) =>
                      `0 0 8px ${alpha(theme.palette.secondary.main, level * 0.5)}`,
                  }}
                />
              ))}
            </Stack>
          ...
          {uploading && (
            <Alert severity="info" sx={{ borderRadius: 3 }}>
              Ses dosyası yükleniyor ve işleme kuyruğa alınıyor…
            </Alert>
          )}
          {!hideActionButton &&
            (recorder.state === "recording" || recorder.state === "paused" ? (
              ...
              <Button
                ...
                onClick={() => void start()}
                disabled={
                  !selectedMeeting || !canRecord || uploading || starting
                }
              >
                {starting
                  ? "Toplantı durumu güncelleniyor…"
                  : !canRecord && selectedMeeting
                    ? "Önce odaya girin"
                    : "Kaydı başlat"}
              </Button>
            ))}
