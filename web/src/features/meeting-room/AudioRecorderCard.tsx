...
export function AudioRecorderCard({ meetings, selectedMeetingId, onMeetingChange, onRecordingStart, onAudioReady, onSaveNotes, uploading, starting, savingNotes }: Props) {
  ...
  return <Card sx={{ height: '100%' }}><CardContent><Stack spacing={2}>
    ...
    {recorder.state === 'recording' ? <Button variant="contained" color="secondary" onClick={() => void stop()}>Kaydı durdur ve gönder</Button> : <Button variant="contained" onClick={() => void start()} disabled={!selectedMeeting || uploading || starting}>{starting ? 'Toplantı durumu güncelleniyor…' : 'Kaydı başlat'}</Button>}
