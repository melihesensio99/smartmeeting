...
export function LoginPage() {
  const auth = useMutation({
    ...
    onSuccess: (result) => {
      localStorage.setItem("smartmeeting-user", result.displayName);
      localStorage.setItem("smartmeeting-user-id", result.userId);
      navigate("/");
    },
  });
  const {
        ...
        <CardContent sx={{ p: { xs: 4, md: 6 } }}>
          <Stack spacing={3.5}>
            ...
            <Typography color="text.secondary" sx={{ fontSize: "1.1rem" }}>
              Toplantılarınızı, notlarınızı ve aksiyonlarınızı tek merkezden
              yönetin.
            </Typography>
            {auth.isError && (
            ...
            >
              {auth.isPending ? "Giriş yapılıyor…" : "Giriş Yap"}
            </Button>
