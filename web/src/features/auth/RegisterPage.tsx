      ...
      <Card sx={{ width: '100%', maxWidth: 650, borderRadius: 4, position: 'relative', zIndex: 1, backgroundColor: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.3)', boxShadow: '0 24px 48px rgba(0,0,0,0.2)', animation: 'sm-fade-in 0.5s ease' }}>
        <CardContent sx={{ p: { xs: 4, md: 6 } }}>
          <Stack spacing={3.5}>
            ...
            <Typography variant="h4" sx={{ fontWeight: 800, background: 'linear-gradient(90deg, #3B246B 0%, #6D52A5 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'inline-block' }}>Hesap Oluştur</Typography>
            ...
            <Button size="large" variant="contained" disabled={registration.isPending} onClick={handleSubmit(submit)} sx={{ py: 1.5, fontSize: '1.1rem', borderRadius: '12px', background: 'linear-gradient(90deg, #3B246B 0%, #6D52A5 100%)', boxShadow: '0 8px 16px rgba(59,36,107,0.3)', transition: 'all 0.2s', '&:hover': { transform: 'translateY(-2px)', boxShadow: '0 12px 20px rgba(59,36,107,0.4)' } }}>{registration.isPending ? 'Hesap oluşturuluyor…' : 'Kayıt Ol'}</Button>
            <Box sx={{ textAlign: 'center' }}>
              <Link component="button" type="button" onClick={() => navigate('/login')} sx={{ color: 'text.secondary', textDecoration: 'none', fontWeight: 500, position: 'relative', '&::after': { content: '""', position: 'absolute', width: '100%', transform: 'scaleX(0)', height: '2px', bottom: -2, left: 0, backgroundColor: 'primary.main', transformOrigin: 'bottom right', transition: 'transform 0.25s ease-out' }, '&:hover::after': { transform: 'scaleX(1)', transformOrigin: 'bottom left' }, '&:hover': { color: 'primary.main' } }}>Zaten hesabınız var mı? Giriş yapın</Link>
            </Box>
          </Stack>
        </CardContent>
      </Card>
