...
function statusOf(meeting: Meeting): string {
...
export function DashboardPage({
  ...
  const todayMeetings = meetings.filter(
    (meeting) => new Date(meeting.startsAt).toDateString() === today,
  );
  const visibleMeetings = [...meetings].sort(
    (first, second) =>
      new Date(second.startsAt).getTime() - new Date(first.startsAt).getTime(),
  );
  const completedSummaries = meetings.filter(
    (meeting) => meeting.summary !== null,
  ).length;
  const openActions = meetings.reduce(
    (total, meeting) =>
      total +
      (meeting.summary?.actionItems.filter((action) => !action.completed)
        .length ?? 0),
    0,
  );
    ...
    <Stack spacing={4}>
      <Stack
        ...
        sx={{
          ...
          p: 5,
          background:
            "radial-gradient(circle at 50% 50%, rgba(255,255,255,0.05) 0%, rgba(0,0,0,0) 100%), linear-gradient(120deg, #0F172A 0%, #1E1B4B 60%, #312E81 100%)",
      ...
      >
        <Box sx={{ position: "relative", zIndex: 1 }}>
          ...
          >
            ...
            — Takvim, AI özetleri ve aksiyonlar tek yerde
          </Typography>
        </Box>
        {canCreateMeetings && (
          <Button
          ...
          <Card>
            <CardContent sx={{ p: 3 }}>
              ...
              >
                ...
                <Chip
                  label={`${visibleMeetings.length} toplantı`}
                  variant="outlined"
                />
              </Stack>
              {visibleMeetings.length ? (
                  ...
                  {visibleMeetings.map((meeting) => (
                    <Card
                      ...
                      sx={{
                        ...
                        borderImage:
                          "linear-gradient(to bottom, #FF6B6B, #ff9999) 1",
                        transition:
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          {meeting.endsAt
                            ? `- ${new Date(meeting.endsAt).toLocaleTimeString("tr-TR", { hour: "2-digit", minute: "2-digit" })}`
                            : ""}
                        </Typography>
                        <Button
                          size="small"
                      ...
                      >
                        <Typography color="text.secondary" variant="body2">
                          Düzenleyen:{" "}
                          {meeting.organizerEmail ?? meeting.organizerId}
                        </Typography>
                        <Chip
                          size="small"
                          label={
                            statusLabels[statusOf(meeting)] ?? statusOf(meeting)
                          }
                          color={statusColors[statusOf(meeting)] ?? "default"}
                        />
                      </Stack>
                    </Card>
                  ))}
                ...
                <Alert severity="info">
                  Henüz dahil olduğunuz bir toplantı bulunmuyor.
                </Alert>
              )}
            </CardContent>
          </Card>
              ...
              {meetings.find((meeting) => meeting.summary)?.summary ? (
                <>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      navigate(
                        `/meetings/${meetings.find((meeting) => meeting.summary)?.id}`,
                      )
                    }
                  >
                    Toplantıyı İncele
