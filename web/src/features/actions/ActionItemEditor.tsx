...
type ActionInput = { description?: string; assigneeUserId: string | null; dueAt: string | null; priority: ActionPriority }
type Props = { action: ActionItem | null; open: boolean; saving: boolean; includeDescription?: boolean; onClose: () => void; onSave: (input: ActionInput) => void }

export function ActionItemEditor({ action, open, saving, includeDescription = false, onClose, onSave }: Props) {
  ...
  return <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm"><DialogTitle>{includeDescription ? 'Manuel aksiyon ata' : 'Aksiyon bilgilerini düzenle'}</DialogTitle><DialogContent><Stack spacing={2} sx={{ pt: 1 }}>{includeDescription && <TextField label="Görev / aksiyon açıklaması" value={description} onChange={(event) => setDescription(event.target.value)} multiline minRows={2} autoFocus slotProps={{ htmlInput: { maxLength: 1000 } }} />}<TextField label="Kayıtlı kullanıcı ara" value={assigneeSearch} onChange={(event) => { setAssigneeSearch(event.target.value); setSelectedAssignee(null) }} slotProps={{ htmlInput: { maxLength: 160 } }} />{users.data?.map((user) => <Button key={user.userId} variant={selectedAssignee?.userId === user.userId ? 'contained' : 'outlined'} onClick={() => { setSelectedAssignee(user); setAssigneeSearch(user.displayName) }} sx={{ justifyContent: 'flex-start' }}>{user.displayName} · {user.email}</Button>)}{assigneeSearch && !selectedAssignee && <Typography variant="caption" color="text.secondary">Sorumluyu kaydetmek için sistemde kayıtlı bir kullanıcı seçin.</Typography>}<TextField label="Termin tarihi" type="date" value={dueAt} onChange={(event) => setDueAt(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} /><TextField select label="Öncelik" value={priority} onChange={(event) => setPriority(event.target.value as ActionPriority)}><MenuItem value="High">Yüksek</MenuItem><MenuItem value="Medium">Orta</MenuItem><MenuItem value="Low">Düşük</MenuItem></TextField></Stack></DialogContent><DialogActions><Button onClick={onClose}>İptal</Button><Button variant="contained" disabled={saving || (includeDescription && !description.trim()) || (assigneeSearch.trim().length > 0 && !selectedAssignee)} onClick={() => onSave({ ...(includeDescription ? { description: description.trim() } : {}), assigneeUserId: selectedAssignee?.userId ?? null, dueAt: dueAt ? new Date(`${dueAt}T23:59:59`).toISOString() : null, priority })}>{saving ? 'Kaydediliyor…' : 'Kaydet'}</Button></DialogActions></Dialog>
}
