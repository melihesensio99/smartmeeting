...
const actionGrid = {
  gridTemplateColumns:
    "150px minmax(260px, 3fr) minmax(180px, 2fr) minmax(180px, 1.5fr) minmax(110px, 1fr) 96px",
};
...
export function ActionsPage({
  ...
  onCreateAction?: (
    meetingId: string,
    input: Required<Pick<ActionInput, "description">> &
      Omit<ActionInput, "description">,
  ) => void;
  creatingAction?: boolean;
  onUpdateAction?: (
    meetingId: string,
    actionItemId: string,
    input: Omit<ActionInput, "description">,
  ) => void;
  ...
  const actions = useMemo(
    () =>
      meetings
        ...
        .filter(
          (item) =>
            isGlobalManager ||
            item.assigneeUserIds.includes(currentUserId ?? "") ||
    ...
    [meetings, currentUserId, filter, isGlobalManager],
  );
  ...
  const editorParticipants = useMemo<Participant[]>(() => {
    ...
    const organizer: Participant = {
      ...
      userId: selectedMeeting.organizerId,
      displayName: selectedMeeting.organizerEmail ?? "Toplantı yöneticisi",
      email: selectedMeeting.organizerEmail ?? "organizer@meeting.local",
      canManageMeeting: true,
      speakerLabel: null,
      speakerMappingStatus: "None",
      speakerConfidence: null,
    };
    return [organizer, ...selectedMeeting.participants].filter(
    ...
    <Stack spacing={4}>
          ...
          {isGlobalManager && onCreateAction && meetings.length > 0 && (
            <>
              <FormControl size="small" sx={{ minWidth: 220 }}>
                <InputLabel id="global-action-meeting-label">
                  Toplantı
                </InputLabel>
                <Select
                  labelId="global-action-meeting-label"
      ...
      <Paper
        sx={{
          ...
          borderRadius: 4,
          boxShadow: (theme) =>
            `0 4px 20px 0 ${alpha(theme.palette.primary.main, 0.05)}`,
        }}
      >
        <Stack
          sx={{
        ...
        <Box sx={{ display: "flex", flexDirection: "column" }}>
          {actions.map((item) => (
            ...
            >
              ...
              >
                <Chip
                  size="small"
                  label={item.completed ? "Tamamlandı" : "Açık"}
                ...
                />
                {!isGlobalManager && !item.completed && onComplete && (
                  <Button
                    size="small"
                    variant="outlined"
                    color="success"
                    onClick={() => onComplete(item.meeting.id, item.id)}
                  >
                    Tamamla
                  </Button>
                )}
              ...
              >
                ...
                {isGlobalManager && onUpdateAction && (
                  <Button
                    size="small"
                    variant="outlined"
                    onClick={() => {
              ...
              {isGlobalManager && onUpdateAction ? (
                ...
                >
                  {item.dueAt
                    ? new Date(item.dueAt).toLocaleDateString("tr-TR")
                    : "Tarih yok"}
                </Button>
              ) : (
          ...
          {actions.length === 0 && (
            <Box sx={{ p: 8, textAlign: "center" }}>
              <Typography sx={{ fontSize: "3rem", mb: 2, opacity: 0.2 }}>
                ☑
              </Typography>
              ...
              <Typography variant="body2" color="text.secondary">
                Bu filtreye uygun herhangi bir aksiyon bulunmuyor.
              </Typography>
            </Box>
          )}
        </Box>
      </Paper>
      {isGlobalManager && onCreateAction && (
        <ActionItemEditor
          key={createOpen ? "create-action" : "create-closed"}
          action={null}
          participants={editorParticipants}
          open={createOpen && Boolean(selectedMeeting)}
          saving={creatingAction}
          includeDescription
          onClose={() => setCreateOpen(false)}
          onSave={(input) => {
            if (!selectedMeeting || !input.description) return;
            onCreateAction(selectedMeeting.id, {
      ...
      {isGlobalManager && onUpdateAction && (
        <ActionItemEditor
          key={editingAction?.id ?? "edit-closed"}
          action={editingAction}
          participants={editorParticipants}
          open={editingAction !== null && Boolean(selectedMeeting)}
          saving={updatingAction}
          onClose={() => setEditingAction(null)}
          onSave={(input) => {
            if (!editingAction || !selectedMeeting) return;
            onUpdateAction(selectedMeeting.id, editingAction.id, {
              assigneeUserIds: input.assigneeUserIds,
              dueAt: input.dueAt,
              priority: input.priority,
            });
...
function getAssigneeLabel(
  ...
  const names = assigneeUserIds
    .map(
      (userId) =>
        people.find((person) => person.userId === userId)?.displayName,
    )
...
function BoxTitle({ isGlobalManager }: { isGlobalManager: boolean }) {
  return (
    <Stack>
      ...
      >
        {isGlobalManager ? "Tüm Aksiyonlar" : "Aksiyonlarım"}
      </Typography>
      <Typography variant="body1" color="text.secondary">
        {isGlobalManager
          ? "Tüm toplantılardaki görevleri yönetin ve yeni aksiyonlar atayın."
          : "Toplantılardan çıkan görevleri tek yerden takip edin."}
      </Typography>
    </Stack>
  );
}
