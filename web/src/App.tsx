...
export function App() {
  ...
  const { meetings, create, startRecording, upload, completeAction, updateAction, updateNotes, mapSpeaker, addParticipant, updateParticipantPermission, removeParticipant, sendEmail } = useMeetings()
  ...
  else page = <MeetingDetailPage meeting={items.find((meeting) => meeting.id === meetingIdFromPath(path))} currentUserId={localStorage.getItem('smartmeeting-user-id')} onComplete={complete} onUpdateAction={(meetingId, actionItemId, input) => updateAction.mutate({ meetingId, actionItemId, ...input })} onSaveNotes={(meetingId, notes) => updateNotes.mutate({ meetingId, notes })} onMapSpeaker={(meetingId, participantId, speakerLabel) => mapSpeaker.mutate({ meetingId, participantId, speakerLabel })} onAddParticipant={(meetingId, input) => addParticipant.mutate({ meetingId, input })} onUpdateParticipantPermission={(meetingId, participantId, canManageMeeting) => updateParticipantPermission.mutate({ meetingId, participantId, canManageMeeting })} onRemoveParticipant={(meetingId, participantId) => removeParticipant.mutate({ meetingId, participantId })} onSendEmail={(meetingId) => sendEmail.mutate(meetingId)} savingNotes={updateNotes.isPending} addingParticipant={addParticipant.isPending} updatingParticipantPermission={updateParticipantPermission.isPending} removingParticipant={removeParticipant.isPending} sendingEmail={sendEmail.isPending} updatingAction={updateAction.isPending} />
