...
import { meetingIdFromPath, navigate, routeFromPath, useAppPath } from './app/navigation'
...
export function App() {
  ...
  const { meetings, create, startRecording, complete: completeMeetingMutation, upload, retryProcessing, completeAction, updateAction, createAction, updateNotes, mapSpeaker, confirmSpeaker, rejectSpeaker, addParticipant, updateParticipantPermission, removeParticipant, leaveMeeting, sendEmail } = useMeetings(!isPublicRoute)
  ...
  else if (route === 'actions') page = <ActionsPage meetings={items} currentUserId={currentUser.data?.userId ?? null} isGlobalManager={currentUser.data?.isGlobalManager ?? false} onComplete={complete} onCreateAction={(meetingId, input) => createAction.mutate({ meetingId, input })} creatingAction={createAction.isPending} onUpdateAction={(meetingId, actionItemId, input) => updateAction.mutate({ meetingId, actionItemId, ...input })} updatingAction={updateAction.isPending} />
