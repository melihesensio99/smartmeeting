...
export function App() {
  ...
  return <Suspense fallback={<PageLoading />}><AppShell route={route} currentUser={currentUser.data}><Stack spacing={3}>{meetings.isError && <Alert severity="error">{getApiErrorMessage(meetings.error, 'Toplantılar yüklenemedi. API adresini ve backend’i kontrol edin.')}</Alert>}{page}</Stack><Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm" slotProps={{ backdrop: { sx: { backgroundColor: 'rgba(20, 12, 45, 0.38)' } } }}><DialogTitle>Yeni toplantı</DialogTitle><DialogContent sx={{ pt: 2 }}><MeetingForm onSubmit={submit} loading={create.isPending} /></DialogContent></Dialog>{route === 'meetings' && currentUser.data?.canCreateMeetings && <Button onClick={() => setOpen(true)} sx={{ position: 'fixed', right: 32, bottom: 32 }} variant="contained">＋ Yeni Toplantı</Button>}{route === 'dashboard' && <Box sx={{ display: 'none' }} />}</AppShell></Suspense>
}
