...
function renderAppShell(currentUser: Parameters<typeof AppShell>[0]['currentUser']) {
...
describe('AppShell', () => {
  it('global manager için tüm toplantı ve aksiyon menülerini gösterir', () => {
    renderAppShell({ userId: 'manager-1', email: 'manager@example.com', displayName: 'Ana Yönetici', isGlobalManager: true, canCreateMeetings: true })
  ...
  it('normal kullanıcı için kişisel menü adlarını korur', () => {
    renderAppShell({ userId: 'user-1', email: 'user@example.com', displayName: 'Standart Kullanıcı', isGlobalManager: false, canCreateMeetings: false })
