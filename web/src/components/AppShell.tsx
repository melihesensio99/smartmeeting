...
export function AppShell({ route, currentUser, children }: { route: AppRoute; currentUser?: CurrentUser; children: React.ReactNode }) {
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
    <AppBar position="fixed" sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}><Toolbar><Stack direction="row" spacing={1.5} sx={{ flexGrow: 1, alignItems: 'center' }}><Box sx={{ width: 36, height: 36, display: 'grid', placeItems: 'center', borderRadius: 2.5, bgcolor: 'secondary.main', color: 'white', fontWeight: 900, letterSpacing: -1 }}>M</Box><Box><Typography variant="h6" sx={{ fontWeight: 800, lineHeight: 1 }}>Meeting</Typography><Typography variant="caption" sx={{ opacity: 0.72 }}>Akıllı toplantı çalışma alanı</Typography></Box></Stack>{currentUser?.isGlobalManager && <Typography color="secondary" sx={{ mr: 3, fontWeight: 700 }}>Global Manager</Typography>}<Typography sx={{ mr: 3 }}>{currentUser?.displayName ?? localStorage.getItem('smartmeeting-user') ?? 'Hesabım'}</Typography><Button color="inherit" variant="outlined" onClick={async () => { await logout(); queryClient.removeQueries({ queryKey: ['current-user'] }); localStorage.removeItem('smartmeeting-user'); localStorage.removeItem('smartmeeting-user-id'); navigate('/login') }}>Çıkış Yap</Button></Toolbar></AppBar>
