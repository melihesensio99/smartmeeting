...
export function AppShell({ route, children }: { route: AppRoute; children: React.ReactNode }) {
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
    <AppBar position="fixed" sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}><Toolbar><Typography variant="h6" sx={{ flexGrow: 1, fontWeight: 800 }}>Toplantı Asistanı</Typography><Typography sx={{ mr: 3 }}>{localStorage.getItem('smartmeeting-user') ?? 'Demo User'}</Typography><Button color="inherit" variant="outlined" onClick={() => { localStorage.removeItem('smartmeeting-access-token'); localStorage.removeItem('smartmeeting-user'); navigate('/login') }}>Çıkış Yap</Button></Toolbar></AppBar>
