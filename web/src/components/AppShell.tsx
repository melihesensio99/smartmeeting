...
export function AppShell({ route, currentUser, meetings = [], children }: { route: AppRoute; currentUser?: CurrentUser; meetings?: Meeting[]; children: React.ReactNode }) {
  ...
  const displayName = currentUser?.displayName ?? localStorage.getItem('smartmeeting-user') ?? 'Hesabım'
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
    <AppBar position="fixed" sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}>
      <Toolbar>
        <Stack direction="row" spacing={1.5} sx={{ flexGrow: 1, minWidth: 0, alignItems: 'center' }}>
          ...
          <Box sx={{ minWidth: 0 }}>
            <Typography variant="h6" noWrap sx={{ fontWeight: 800, lineHeight: 1, textShadow: '0px 2px 4px rgba(0,0,0,0.1)' }}>
              Meeting
            </Typography>
            <Typography variant="caption" noWrap sx={{ display: 'block', opacity: 0.82 }}>
        ...
        {currentUser?.isGlobalManager && (
          <Typography color="secondary" sx={{ mr: 3, fontWeight: 700, whiteSpace: 'nowrap' }}>
            Global Manager
          </Typography>
        )}
        
        <Stack direction="row" spacing={1} sx={{ mr: 3, alignItems: 'center' }}>
          <Box sx={{ 
            width: 28, 
            height: 28, 
            borderRadius: '50%', 
    ...
    >
      <List sx={{ flexGrow: 1, px: 2, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
        {menu.map((item) => {
          ...
          return (
            <ListItemButton 
              ...
              sx={{ 
                ...
                bgcolor: isSelected ? (theme) => alpha(theme.palette.secondary.main, 0.08) : 'transparent',
                position: 'relative',
                '&::before': isSelected ? {
                  content: '""',
                  position: 'absolute',
            ...
            >
              <Box sx={{ 
                ...
                alignItems: 'center', 
                justifyContent: 'center',
                bgcolor: isSelected ? (theme) => alpha(theme.palette.secondary.main, 0.15) : (theme) => alpha(theme.palette.text.primary, 0.04),
    ...
    <Box component="main" sx={{ ml: `${drawerWidth}px`, pt: 11, px: { xs: 2, md: 4 }, pb: 5 }}>
      {children}
    </Box>
  </Box>
}
