...
export function AppShell({ route, currentUser, children }: { route: AppRoute; currentUser?: CurrentUser; children: React.ReactNode }) {
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
    <AppBar position="fixed" sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}><Toolbar><Typography variant="h6" sx={{ flexGrow: 1, fontWeight: 800 }}>Toplantı Asistanı</Typography>{currentUser?.isGlobalManager && <Typography color="secondary" sx={{ mr: 3, fontWeight: 700 }}>Global Manager</Typography>}<Typography sx={{ mr: 3 }}>{currentUser?.displayName ?? localStorage.getItem('smartmeeting-user') ?? 'Hesabım'}</Typography><Button color="inherit" variant="outlined" onClick={async () => { await logout(); localStorage.removeItem('smartmeeting-user'); localStorage.removeItem('smartmeeting-user-id'); navigate('/login') }}>Çıkış Yap</Button></Toolbar></AppBar>
